...
// @version      1.2.0
// @description  只保留在线观众数量与纯评论，并自动处理礼物消息和节能暂停提示。
// @author       local
...
(function () {
    ...
    var HIDE_GIFT_MESSAGES = true;
    var KEEP_ALIVE_INTERVAL = 45000;
    ...
    function findKnownChatItem(el, root) {
        for (var current = el, depth = 0; current && current !== root && depth < 5; current = current.parentElement, depth++) {
            var token = tokenOf(current);
            if (/webcast-chatroom___item|message[-_ ]?item|chat[-_ ]?item|comment[-_ ]?item/i.test(token)) {
                return current;
            }
        }
        return el;
    ...
    function hideGiftMessages(root) {
        if (!HIDE_GIFT_MESSAGES) return;
        var giftPattern = /送出|赠送了?|送给主播|为主播加了|点亮了?粉丝灯牌|加入了?粉丝团|开通了?(?:会员|守护)|升级至/i;
        var nodes = new Set();

        try {
            root.querySelectorAll('[data-e2e*="gift" i], [class*="gift" i]').forEach(function (el) {
                nodes.add(el);
            });
        } catch (error) { /* ignore unsupported selector */ }
        ...
        itemSelectors.forEach(function (selector) {
            try {
                root.querySelectorAll(selector).forEach(function (el) {
                    if (giftPattern.test(normalizeText(el.textContent))) nodes.add(el);
                });
            } catch (error) { /* ignore unsupported selector */ }
        });

        // 文本兜底仅隐藏命中的消息项，不扩大到整个评论列表。
        root.querySelectorAll('span, p, div').forEach(function (el) {
            var text = normalizeText(el.textContent);
            if (text.length > 0 && text.length <= 140 && giftPattern.test(text)) {
                nodes.add(findKnownChatItem(el, root));
            }
        });

        nodes.forEach(function (el) {
            if (el && el !== root && !el.contains(root)) hideElement(el);
        });
    }

    function dismissEnergySavingPrompt() {
        if (!state.enabled || !isLivePage()) return false;
        var promptPattern = /节能模式|长时间未操作|已暂停播放|暂停播放以节省/i;
        var actionPattern = /继续播放|恢复播放|继续观看|退出节能模式|点击恢复/i;
        var clicked = false;
        var candidates = Array.from(document.querySelectorAll('button, [role="button"], a'));

        // 部分版本把操作文字绑定在普通元素上，只把精确的操作文案作为兜底，避免点到整层弹窗。
        document.querySelectorAll('div, span').forEach(function (el) {
            if (/^(继续播放|恢复播放|继续观看|退出节能模式|点击恢复)$/.test(normalizeText(el.textContent))) {
                candidates.push(el);
            }
        });

        candidates.forEach(function (el) {
            if (clicked || el === state.button || el === state.audienceBar) return;
            var actionText = normalizeText(el.textContent);
            if (!actionText || actionText.length > 30 || !actionPattern.test(actionText)) return;

            var relatedToPrompt = promptPattern.test(actionText);
            for (var parent = el.parentElement, depth = 0; parent && depth < 6 && !relatedToPrompt; parent = parent.parentElement, depth++) {
                var parentText = normalizeText(parent.textContent);
                if (parentText.length <= 180 && promptPattern.test(parentText)) relatedToPrompt = true;
            }
            if (!relatedToPrompt && !/^继续播放$/.test(actionText)) return;

            try {
                el.click();
                clicked = true;
            } catch (error) { /* retry on the next scan */ }
        });
        return clicked;
    }

    function sendKeepAliveSignal() {
        if (!state.enabled || !isLivePage()) return;
        var target = state.root && state.root.isConnected ? state.root : document.body;
        if (!target) return;
        ...
        try {
            target.dispatchEvent(new MouseEvent('mousemove', {
                bubbles: true,
                cancelable: false,
                clientX: Math.max(1, window.innerWidth - 2),
                clientY: Math.max(1, window.innerHeight - 2)
            }));
            target.dispatchEvent(new Event('pointermove', { bubbles: true }));
            window.dispatchEvent(new Event('focus'));
        } catch (error) { /* keep the prompt detector active even on older browsers */ }
        dismissEnergySavingPrompt();
    }
    ...
    function boot() {
        ...
        state.keepAliveTimer = setInterval(sendKeepAliveSignal, KEEP_ALIVE_INTERVAL);
    ...
    document.addEventListener('visibilitychange', function () {
        if (!document.hidden) setTimeout(sendKeepAliveSignal, 0);
    });
    window.addEventListener('focus', function (event) {
        // sendKeepAliveSignal 自己会派发一次模拟 focus；只响应真实的窗口聚焦，避免递归。
        if (event.isTrusted) setTimeout(sendKeepAliveSignal, 0);
    });
    window.addEventListener('pageshow', function () { setTimeout(sendKeepAliveSignal, 0); });
